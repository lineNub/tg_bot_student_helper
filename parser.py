...
def ConnectPostgres(all_people_information):
    ...
    connection = pg8000.native.Connection('postgres', password = '464646', host = '127.0.0.1', port = '5432', database = 'bot')

    try:
        connection.run("DROP TABLE IF EXISTS teachers;")
        connection.run("CREATE TABLE teachers (fullname text NOT NULL, contacts text, consultations text, CONSTRAINT pkey_fullname PRIMARY KEY (fullname));")
        for people in all_people_information:
            contacts = ''
            consultations = ''
            for i in range(1,len(people)):
                if ('Время начала' in people[i]):
                    ...
                    contacts += people[i] + "\n"
            try:
                connection.run(f"INSERT INTO teachers VALUES ('{people[0]}', '{contacts}', '{consultations}');")
            except:
