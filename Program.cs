...
namespace Bot_Telegram
{
    class Program
    {
                    ...
                    case UpdateType.Message:
                        {
                            ...
                            switch (message.Type){     
                                ...
                                case MessageType.Text:{
                                        ...
                                        {
                                            await InsertIntoLogs(15, 1);
                                            await SelectFromLogs(15);
                                            Console.WriteLine($"val is {flag}");
        ...
        private static async Task InsertIntoLogs(uint user_id, int val)
        {
            if (sql.State == ConnectionState.Closed)
            {
                sql.Open();
                NpgsqlCommand command = new NpgsqlCommand($"INSERT INTO logs (chat_id, val) VALUES ('{user_id}', {val})", sql);
                await command.ExecuteNonQueryAsync();
                sql.Close();
            }
        }

        private static async Task SelectFromLogs(uint chat_id)
        {
            if (sql.State != ConnectionState.Open)
            {
                ...
                //            {
                //                Console.WriteLine( reader.GetInt32(i).ToString());//Получаем запись и переводим её в строку
